...
% TODO: place all drawn objects into their own part of the handles structure
handles.objects = [];
...
handles.menu.icatoggle = uimenu(handles.menu.options,...
    'label', 'show channels/components',...
    'checked', 'off' ,...
    'accelerator', 't', ...
...
handles.menu.spectral_toggle = uimenu(handles.menu.options,...
    'label', 'spectral power toggle',...
    'checked', 'off' ,...
    'callback', {@fcn_options, 'spectral_toggle'});
...
switch type
    ...
    case 'spectral_toggle'
        % show the spectral power (currently only delta/beta) of entire dataset
        
        switch get(handles.menu.spectral_toggle, 'checked')
            case 'off'
                % set the toggle to the other state
                set(handles.menu.spectral_toggle, 'checked', 'on');
                
                % check for existing spectral power and plot
                if isfield(handles.objects, 'spectral_line')
                    set(handles.objects.spectral_line, 'visible', 'on');
                    
                else
                    % calculate the spectral power
                    [temp_data, time_points] = csc_epoched_fft(EEG, 1, 30);
                    
                    % rescale the data to fit well in the axes
                    y_lim = get(handles.spike_ax, 'ylim');
                    scaling_factor = y_lim(2) / 4; % make it so 4 median deviations fit in the axes (no idea if this is robust!)
                    temp_data = abs((temp_data .* scaling_factor) - y_lim(2));
                    
                    
                    % plot in the events axis
                    handles.objects.spectral_line = plot(time_points, temp_data, ...
                        'color', handles.colorscheme.fg_col_3,...
                        'linewidth', 2, ...
                        'parent', handles.spike_ax);
                    uistack(handles.objects.spectral_line, 'bottom');
                end
                
            case 'on'
                % set the toggle to the other state
                set(handles.menu.spectral_toggle, 'checked', 'off');
                
                % set to invisible
                set(handles.objects.spectral_line, 'visible', 'off');
